...
const CheckoutReviewForm: React.FC = () => {
  ...
  const handleUpdateClick = async (rowID: string) => {
    ...
    try {
      const response = await fetch(
        "http://ec2-3-222-139-134.compute-1.amazonaws.com:27015/payment/RentalCart/Update",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const handleMakePaymentClick = async () => {
    ...
    try {
      const response = await fetch(
        "http://ec2-3-222-139-134.compute-1.amazonaws.com:27015/payment/Checkout/Pay",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
